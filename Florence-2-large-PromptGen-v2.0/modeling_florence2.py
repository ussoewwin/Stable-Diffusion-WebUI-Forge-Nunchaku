...
class DaViT(nn.Module):
    ...
    def __init__(
        ...
        dpr = [float(drop_path_rate) * i / (sum(depths)*2 - 1) if sum(depths)*2 > 1 else 0.0 for i in range(sum(depths)*2)]
    ...
    def forward_features_unpool(self, x):
        ...
        Returns:
            x: output tensor
            input_size: tuple (H, W) of spatial dimensions
        """
    ...
    def forward_features(self, x):
        x, _ = self.forward_features_unpool(x)
...
class Florence2LanguagePreTrainedModel(PreTrainedModel):
    ...
    # transformers 5.0.0 attribute names
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    # transformers 5.0.0: dict format {target: source}
    _tied_weights_keys = {
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    ...
    # transformers 5.0.0: dict format {target: source} - matches BartForConditionalGeneration pattern
    _tied_weights_keys = {
...
class Florence2PreTrainedModel(PreTrainedModel):
    ...
    supports_gradient_checkpointing = True
    _skip_keys_device_placement = "past_key_values"
    # transformers 5.0.0 attribute names (class-level, not properties)
    _supports_flash_attn = True
    _supports_sdpa = True
    _supports_flex_attn = False
...
class Florence2VisionModel(Florence2PreTrainedModel):
    ...
    def forward(self, pixel_values):
        if len(pixel_values.shape) == 4:
            x, spatial_size = self.vision_tower.forward_features_unpool(pixel_values)
        else:
...
class Florence2VisionModelWithProjection(Florence2PreTrainedModel):
    ...
    def forward(self, pixel_values):
        ...
        if self.image_pos_embed is not None:
            ...
            # Use actual spatial dimensions from vision tower instead of assuming square
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    # Inherits _supports_flash_attn, _supports_sdpa, _supports_flex_attn from Florence2PreTrainedModel
    ...
    def __init__(self, config: Florence2Config):
        ...
        if language_model._tied_weights_keys is not None:
            # transformers 5.0+ expects dict format {target: source}
    ...
    def _merge_input_ids_with_image_features(
        ...
        # attention_mask must be long dtype for transformers 5.0.0
        image_attention_mask = torch.ones(batch_size, image_token_length, dtype=torch.long, device=device)
        ...
        task_prefix_attention_mask = torch.ones(batch_size, task_prefix_embeds.size(1), dtype=torch.long, device=device)
    ...
    def forward(
        ...
        # attention_mask should stay as long dtype, do NOT convert to inputs_embeds.dtype
    ...
    def generate(
        ...
        **kwargs
        ...
        # For encoder-decoder models in transformers 5.0+, we need to:
        # 1. Compute encoder outputs first
        # 2. Pass encoder_outputs to language_model.generate()
        ...
        # attention_mask should stay as long dtype for transformers 5.0.0
        # Do NOT convert to inputs_embeds.dtype
        
        # Run through the encoder to get encoder_outputs
        ...
        # Now call generate with encoder_outputs
