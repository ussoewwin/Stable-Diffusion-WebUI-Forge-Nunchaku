...
5. [Full source of new files](#5-full-source-of-new-files)
6. [Full source of modified files](#6-full-source-of-modified-files) (entire files at commit `a8f6373`)
7. [Full source of removed files](#7-full-source-of-removed-files) (at commit `95832e0`)
8. [Recommended load setup and troubleshooting](#8-recommended-load-setup-and-troubleshooting)
9. [Release documentation](#9-release-documentation-reference)
...
[Anima](https://huggingface.co/circlestone-labs/Anima) (circlestone-labs) is **Flow Matching + Cosmos DiT (3D) + Qwen3 0.6B + T5 tokens + Wan/Qwen Image VAE**. Even for still images, DiT input is **5D** `(B,C,T,H,W)` with `T=1`.

### 1.2 Pipeline in this repo (summary)

Loading a checkpoint (e.g. `waiANIMA_pw3.safetensors`) plus Additional files (`qwen_3_06b_base.safetensors`, `qwen_image_vae.safetensors`) reaches txt2img along this path:
...
### 1.3 What happens at load time (detail)

1. **Detection**: `detection.py` checks `x_embedder.proj.1` and `llm_adapter`, sets `image_model=anima`. Lumina2 is separated by presence of `noise_refiner`.
2. **Fine config**: `comfy_anima.py` infers block count, `in_channels` (68 → wai family), RoPE scale, etc.
3. **Split**: `loader.split_state_dict` merges **Additional** TE/VAE via `replace_state_dict`, then splits into transformer / text_encoder / vae. **`llm_adapter` weights move from the UNet dict to the TE dict** (wrong placement breaks quality). Typical wai ckpt has DiT only; TE and VAE come from Additional files.
4. **Build**: Even when labeled `CosmosTransformer3DModel`, only on Anima guess is `backend.nn.anima.Anima` instantiated.
5. **Generation**: Prompts go through `AnimaTextProcessingEngine` → cross-attn length 512. Denoising uses `KModel` as the 4D↔5D bridge.

### 1.4 Design constraints in Forge Nunchaku

Anima runs as a **native Forge pipeline** (no Comfy `BaseModel`). All loader/engine changes sit behind Anima-only branches so existing paths — Nunchaku Qwen Image, Z-Image, SDXL, Flux, etc. — stay isolated.
...
| Path | Role |
|------|------|
| `backend/diffusion_engine/anima.py` | Forge engine (TE/VAE/UNet/conds/Shift/decode) |
| `backend/nn/comfy_anima.py` | DiT config inference and key remap from checkpoint |
| `backend/text_processing/anima_engine.py` | Prompt → cross-attn (Qwen+T5+emphasis) |
| `backend/nn/anima.py` | Native DiT + LLMAdapter |
| `backend/huggingface/circlestone-labs/Anima/model_index.json` | Local HF pipeline manifest |
...
| `backend/huggingface/circlestone-labs/Anima/transformer/config.json` | Diffusers class name stub |
| `backend/huggingface/circlestone-labs/Anima/vae/config.json` | Qwen Image VAE metadata |
| `backend/huggingface/circlestone-labs/Anima/tokenizer/tokenizer_config.json` | Qwen2Tokenizer config |
| `backend/huggingface/circlestone-labs/Anima/tokenizer_2/tokenizer_config.json` | T5Tokenizer config |
| `backend/huggingface/circlestone-labs/Anima/tokenizer_2/special_tokens_map.json` | T5 special tokens |
| `backend/huggingface/circlestone-labs/Anima/tokenizer/merges.txt` | (**Large** 151,388 lines — see [§5.3](#53-large-tokenizer-assets); HF bundle, not reproduced here) |
| `backend/huggingface/circlestone-labs/Anima/tokenizer/vocab.json` | (**Large** single line — see [§5.3](#53-large-tokenizer-assets); HF bundle, not reproduced here) |
| `backend/huggingface/circlestone-labs/Anima/tokenizer_2/tokenizer.json` | (**Large** 129,428 lines — see [§5.3](#53-large-tokenizer-assets); HF bundle, not reproduced here) |

### 2.2 Modified (M)

| Path | Role |
|------|------|
| `backend/loader.py` | Load/split/Anima branching hub |
| `backend/modules/k_model.py` | 4D↔5D during sampling |
| `backend/nn/llm/llama.py` | Qwen3_06B added |
| `backend/sampling/condition.py` | Optional vector cond |
| `modules/processing.py` | batch/forge_objects stability |
| `modules/processing_scripts/sampler.py` | Sampler arg coercion |
| `modules/sd_samplers.py` | Sampler name int→str |
| `modules/txt2img.py` | Default sampler/scheduler |
| `modules_forge/main_entry.py` | UI preset Anima |
| `modules_forge/packages/huggingface_guess/detection.py` | Early Anima detect |
| `modules_forge/packages/huggingface_guess/model_list.py` | Anima* classes |
| `modules_forge/presets.py` | Anima defaults |

### 2.3 Removed (D)

- `backend/nn/comfy_model_base.py` (unused Comfy path; gone at `a8f6373`)
- `backend/nn/comfy_model_detection.py` (unused Comfy path; gone at `a8f6373`)

---

## 3. Architecture

### 3.1 Isolation (avoid breaking other models)

| Gate | Condition | Effect |
|--------|-----------|--------|
| `split_state_dict` | `detect_unet_config` returns `image_model==anima` | Anima-specific guess before generic `guess()` |
| `load_huggingface_component` Qwen3 | `isinstance(guess, Anima*)` | **Qwen3_06B** only; otherwise legacy **Qwen3_4B** |
| `CosmosTransformer3DModel` loader | Anima guess only | Builds `backend.nn.anima.Anima` |
| `replace_state_dict` | hidden 1024 and repo contains `Anima` | Prefix extra TE as `qwen3_06b` |
| `KModel._is_anima_diffusion_model` | `isinstance(..., ForgeAnima)` | 5D wrap for native Anima only |

### 3.2 Checkpoint detection

1. **`detection.detect_unet_config`**: `x_embedder.proj.1` + `llm_adapter` prefix separates Anima from Lumina2/Cosmos. Lumina2 requires `noise_refiner`.
2. **`comfy_anima._guess_unet_config`**: Detailed inference like Comfy `model_detection` (`in_channels` 68 → `AnimaWai68`, etc.).
3. **`_remap_anima_state_dict`**: Reconciles main DiT `o_proj` ↔ `output_proj` and llm_adapter naming.

### 3.3 Text encoding

- **Qwen tokens**: `AnimaTextProcessingEngine` embeds → `Qwen3_06B` body.
- **T5 tokens**: Passed as `t5xxl_ids` into `LLMAdapter` to build the 1024-d cross-attn sequence.
- **`uses_emphasis`**: `emphasis.py` unchanged; engine uses `_uses_emphasis()` for compatibility.
- **Padding**: cross-attn padded to length 512 (DiT expectation).

### 3.4 Shift (distilled CFG)

The **Anima** preset **Shift** slider is read in `get_learned_conditioning` as `prompt.distilled_cfg_scale` and applied via `predictor.set_parameters(shift=...)`. **Runtime UI value** overrides the fixed 3.0 in `model_list`.

### 3.5 5D tensor wrap

Cosmos/Anima DiT expects `(B, C, T, H, W)`. Forge latents are `(B, C, H, W)`, so `KModel` **inserts/removes T=1**. Only when `isinstance(backend.nn.anima.Anima)`; SDXL and Qwen Image 2D transformers are untouched.

### 3.6 Why the Comfy path was dropped

Early work tried Comfy `BaseModel`, but (1) `o_proj` / `output_proj` naming, (2) `llm_adapter` spanning TE and UNet, and (3) Wan normalize around decode were easier and higher quality on **native Forge**. `comfy_model_base.py` etc. were removed; this guide’s §5–§6 path is the only one.

### 3.7 Suggested reading order

`detection.py` → `comfy_anima.py` → `model_list.py` → `loader.py` → `diffusion_engine/anima.py` → `anima_engine.py` → `anima.py` → `llama.py` → `k_model.py`. UI files last.

---

## 4. File index

§5 and §6 place **notes before each file**, then the **full source at `a8f6373`**. Quick index:

| File | Summary | Section |
|------|---------|---------|
| `diffusion_engine/anima.py` | Inference hub | §5 start |
| `nn/comfy_anima.py` | ckpt → DiT kwargs | §5 |
| `text_processing/anima_engine.py` | Prompt handling | §5 |
| `nn/anima.py` | DiT + LLMAdapter | §5 |
| `loader.py` | Load hub | §6 start |
| `detection.py` | Early type detect | §6 |
| `model_list.py` | Anima* classes | §6 |
| `k_model.py` | 4D↔5D | §6 |
| `llama.py` | Qwen3_06B | §6 |
| `condition.py` | Optional vector | §6 |
| `processing.py` etc. | Gradio stability | §6 later |

---

## 5. Full source of new files
...
## 6. Full source of modified files
...
## 7. Full source of removed files
...
## 8. Recommended load setup and troubleshooting

### 8.1 Recommended layout

| Type | Example | Notes |
|------|---------|-------|
| Checkpoint | `waiANIMA_pw3.safetensors` | DiT weights; may include `llm_adapter` |
| Additional TE | `qwen_3_06b_base.safetensors` | Required if ckpt has no TE |
| Additional VAE | `qwen_image_vae.safetensors` | For decode |
| UI Preset | **Anima** | Shift slider enabled |

### 8.2 Signs of a successful load

- `StateDict Keys:` lists `transformer`, `text_encoder` (and `vae`)  
- Guess is one of `AnimaWai68` / `AnimaBase` / `Anima`  
- txt2img finishes without exceptions  
...
| Decode failure | VAE not wired | `is_wan=True` + `process_in/out` |
| Poor quality | Comfy path / TE mismatch | Native UNet + Qwen3_06B + move `llm_adapter` |
| Misclassified as Lumina | `cap_embedder` only | Require `noise_refiner` to separate Lumina2 |

---

## 9. Release documentation (reference)

After implementation commit `a8f6373`, **Version 1.7.0** release material (`95832e0`..`bbbc6b9`, tag `v1.7.0`):

- `Changelog/CHANGELOG.md` — v1.7.0 Anima entry
- `README.md` — Anima feature + `png/anima.png`

---

*Notes: hand-written (§1–§4 and before each file in §5–§6). Full code: commit `a8f6373` (removed files: `95832e0`). **Version 1.7.0** native pipeline. For v1.7.1 Comfy import see `ANIMA_COMFY_IMPORT_GUIDE_7061e66.md`.*
