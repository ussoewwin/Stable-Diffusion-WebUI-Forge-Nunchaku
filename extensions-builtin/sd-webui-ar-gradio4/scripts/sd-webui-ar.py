...
def create_res_button(res=(512, 512), **kwargs):
    """Create a resolution button with the specified resolution"""
    if 'elem_classes' not in kwargs:
        kwargs['elem_classes'] = ["tool"]
    ...
    button.w, button.h = res
    return button

def create_ar_button(ar=1.0, value=None, **kwargs):
    """Create an aspect ratio button with the specified aspect ratio"""
    if 'elem_classes' not in kwargs:
        kwargs['elem_classes'] = ["tool"]
    if 'variant' not in kwargs:
        kwargs['variant'] = "secondary"
...
def apply_aspect_ratio(ar, w, h):
    """Apply aspect ratio to width and height"""
    if ar > 1.0:  # fix height, change width
        w = ar * h
    elif ar < 1.0:  # fix width, change height
        h = w / ar
    ...
    def ui(self, is_img2img):
        with gr.Column(
            ...
            with gr.Row(
                ...
                btns = [
                    create_ar_button(ar=ar, value=label)
                    for ar, label in zip(
                        self.aspect_ratios,
                        self.aspect_ratio_labels,
                    )
                ...
                with contextlib.suppress(AttributeError):
                    for b in btns:
                        ...
                        b.click(
                            lambda w, h, ar=b.ar: apply_aspect_ratio(ar, w, h),
                            inputs=resolution,
                            outputs=resolution,
                        )
            ...
            with gr.Row(
                ...
                btns = [
                    create_res_button(res=res, value=label)
                ...
                with contextlib.suppress(AttributeError):
                    for b in btns:
                        ...
                        b.click(
                            lambda w, h, res=b.w, res_h=b.h: [res, res_h],
                            outputs=resolution,
                        )
